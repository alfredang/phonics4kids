...
### Deployment & Security
-   Vite deployment to static hosts (GitHub Pages) requires passing secrets via GitHub Actions.
-   Use `process.env` mapping in `vite.config.ts` to bake secrets into the production build during CI/CD.
